...
WindowManager::WindowManager()
{
  ...
  if (xcb_connection_has_error(conn_)) utils_.die("xcb_connection");

  setup_ = xcb_get_setup(conn_);
  
  iter_ = xcb_setup_roots_iterator(setup_);
  for (int i = 0; i < screen_n_; ++i) xcb_screen_next(&iter_);
  screen_ = iter_.data;
  if (!screen_) utils_.die("screen");

  root_ = screen_->root;

  keysyms_ = xcb_key_symbols_alloc(conn_);
  if (!keysyms_) utils_.die("xcb_key_symbols_alloc");
  ...
  while ((event_ = xcb_wait_for_event(conn_)))
  {
    switch (event_->response_type & ~0x80)
    {
      ...
      case XCB_KEY_PRESS:
      {
        ...
        {
          alt_ret_down_ = true;
          // utils_.spawn("env -u WAYLAND_DISPLAY kitty --linux-display-server x11");
          utils_.spawn("kitty");
        }
      ...
      case XCB_DESTROY_NOTIFY:
      {
        ...
        if (focused_ == e->window) focused_ = XCB_WINDOW_NONE;
